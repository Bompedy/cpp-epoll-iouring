...
int main() {
    ...
    constexpr unsigned int num_instances = 2;
   ...
   Consensus<256> node0(IOType::EPOLL, Algorithm::MULTI_PAXOS, num_instances, 0, num_conn_per_peer, pipes_per_instance, peer_addrs, 10000);
   Consensus<256> node1(IOType::EPOLL, Algorithm::MULTI_PAXOS, num_instances, 1, num_conn_per_peer, pipes_per_instance, peer_addrs, 10000);
   while (RUNNING.load()) {
       pause();
   }
